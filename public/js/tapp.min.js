...
const player_settings = {
  constraints: {
    height: 0, // (window.innerHeight * 0.025),
    width: 0,
  },
...
let init_player = function() {
  ...
  let player_state_id = setInterval(function() {
    ...
    if ("Playing" === state.playback || "Ready" === state.playback) {
      clearInterval(player_state_id);
      if (!window.api) {
        toggle_alert(!1);
      }
      
      document.getElementById("vid").style.display = "block";
      player.setQuality(player.getQualities()[1].group);
    }
...
if (window.api) {
  ...
  // NOT VIEWING FROM ELECTRON
  // document.removeChild(document.getElementsByTagName("header")[0]);
